好的，这是基于**第一个脚本（完整国家分类版）**修改的版本。

**主要修改点：**
1.  **统一前缀**：设定为 **`const PRE = "优化浏览-";`**。
2.  **保留国家分类**：依然会识别香港、日本、美国等地区，并将“优化浏览-”放在最前面（例如：“优化浏览-香港 01”、“优化浏览-美国 01”）。
3.  **功能保持**：保留了之前提到的深港/沪港硬锁为香港、清理多余信息节点等功能。

```javascript
/**
 * Sub-Store rename script (Optimized Browse Prefix + Country Categorization)
 * 
 * 特性：
 * 1) ✅ 强制统一前缀 "优化浏览-"
 * 2) 🌍 依然进行国家/地区分类（香港、日本、美国...）
 * 3) 🛠 包含之前的修复：清理无效节点、修正 HK 识别、布尔参数修复等
 */
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    if (findKey?.[1]) {
      ...
      // ✅ 强制将 PRE (优化浏览-) 置于首位
      keyover = [PRE].concat(firstName, usflag, nNames, findKeyValue, retainKey, ikey, ikeys).filter(Boolean);
      e.name = keyover.join(FGF);
    } else {
      // ✅ 未匹配到国家时，如果设置保留(nm=true)，同样添加前缀
      if (nm) {
          e.name = PRE + FGF + (FNAME ? FNAME + FGF : "") + e.name; 
      } else {
          e.name = null;
      }
    }
