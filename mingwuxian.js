...
**主要修改点：**
1.  **新增 `const PRE = "无限-";`**：定义了统一前缀变量。
2.  **强制添加前缀**：在生成最终节点名称的逻辑中，将 `PRE` 作为第一个元素加入，确保无论是否有国旗或额外名称，"无限-" 永远在最前面。
3.  **未匹配兜底**：如果启用了 `nm=true` (保留未匹配节点)，也会自动加上该前缀。
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    if (findKey?.[1]) {
      ...
      // ✅ [修改] 强制将 PRE 加入到 keyover 数组的最前端
      // 原代码：keyover = keyover.concat(...)
